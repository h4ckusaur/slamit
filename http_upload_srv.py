...
class UploadAndServeHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not content_type or "multipart/form-data" not in content_type:
            ...
            self.end_headers()
            self.wfile.write(
                b"Invalid Content-Type for upload. Expected multipart/form-data."
            )
            return
        ...
        if "file" in form and form["file"].filename:
            ...
            if "folder" in form and form["folder"].value:
                ...
                if not requested_path.startswith(BASE_DIR):
                    ...
                    self.end_headers()
                    self.wfile.write(
                        b"Access denied: Invalid folder path. Path must be within the base directory."
                    )
                    return
            ...
            self.end_headers()
            response_message = (
                f'File "{filename}" uploaded to "{target_dir}" successfully.\n\n'
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=7979, help="Port to listen on (default: 7979)"
    )
